...
class ManagedAgentWorkload(Workload):
    ...
    async def setup(self, ctx: TrialContext) -> None:
        ...
        if self.use_sandbox_tools:
            ...
            system = (
                "For every user request, call bash exactly once with command "
                "`printf AGENTD_PERF_SANDBOX_OK`, then answer exactly AGENTD_PERF_OK."
            )
    ...
    async def fire(self, target, client: httpx.AsyncClient, case, run_id: str) -> Outcome:
        ...
        try:
            ...
            while time.monotonic() < deadline:
                ...
                if messages > slot.agent_messages:
                    latest_message = next(
                        event for event in reversed(data) if event.get("type") == "agent.message"
                    )
                    if self.use_sandbox_tools and "AGENTD_PERF_OK" not in str(
                        latest_message.get("content", "")
                    ):
                        meta["unexpected_response"] = latest_message.get("content")
                        return Outcome(
                            status=500,
                            duration_ms=(time.monotonic() - started) * 1000,
                            nbytes=nbytes,

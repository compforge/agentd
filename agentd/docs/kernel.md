...
1. **资源控制**：agentd 作为 Control Plane，通过 Claude Managed Agents 兼容 API 管理 Agent、
   Environment、Session 和 Event，维护 Agentlet 实例、容量、Assignment 与全局 Control State。
...
Claude-compatible API / Baton / scheduled trigger
       ...
       resource / scheduling / routing / reconcile
              │ Managed API + Assignment
                         ▼
                      Agentlet
      runtime / freeze / restore / observed state
           │              │              │
           ▼              ▼              ▼
...
- **Assignment**：Control Plane 授予某个 Agentlet 的临时执行归属，带 generation、lease 与
  fencing，不是产品身份。
...
3. Scheduler 根据 Agentlet 的能力、可分配量和 reservation 选择实例，agentd 持久化 Assignment
   和 Session Binding；
...
6. Harness 输出投影为持久化 Event，Agentlet 上报 observed state；
7. agentd 校验 fencing 后提交 Control State；空闲或等待中的 Session 释放 Harness runtime，
   Sandbox 按引擎能力保留、休眠或回收，agentd 释放或更新 Assignment。
...
附加不进入公开 schema 的 Assignment 与 fencing 元数据；调用方只依赖 agentd API。
...
Control State 拥有实例、容量、Assignment 和 Session 当前决策，其模型与条件更新规则定义在

...
| Event read after assignment release | Session Observer → Assignment release → shared Ledger → Event projection | SQLite + deterministic Agentlet HTTP substrate |
...
proves that persisted Event reads remain available directly from the shared Ledger after that
Assignment has been released, without routing through a Worker.

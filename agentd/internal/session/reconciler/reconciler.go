...
// +case:id=control_plane_restart_before_dispatch,desc=`The Control Plane restarts immediately after accepting durable Session input`,input=`restart-control-plane-after-ingress-before-dispatch`,expect=`the replacement Control Plane rediscovers demand and produces one answer`,forbid=`lost durable input or duplicate user and agent messages`,group=system
// +case:id=control_plane_restart_during_turn,desc=`The Control Plane restarts while Agentlet is executing a Turn`,input=`restart-control-plane-during-running-turn`,expect=`Agentlet finishes independently and the replacement Control Plane converges the same Session`,forbid=`Worker replacement, duplicate output, or Session termination`,group=system
// +case:id=control_plane_multi_replica_failover,desc=`One of two active Control Plane replicas disappears while durable Sessions continue`,input=`delete-one-of-two-control-plane-replicas`,expect=`the surviving replica accepts demand and the replacement rejoins without duplicate execution`,forbid=`global-leader dependency, Worker churn, lost input, or duplicate output`,group=system

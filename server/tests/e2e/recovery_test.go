...
func (*sqliteRecoveryHarness) PrepareSession(_ context.Context, session app.Session) (string, error) {
...
func (h *sqliteRecoveryHarness) Run(
	...
	records, err := h.state.Load(ctx, session.Control.ResumeRef)
	if err != nil {
		return app.TurnResult{}, fmt.Errorf("load SQLite E2E harness state: %w", err)
	}
	revision := int64(-1)
	committed := false
	for _, record := range records {
		...
		if err := json.Unmarshal(record.Data, &stored); err != nil {
			return app.TurnResult{ResumeRevision: revision}, fmt.Errorf("decode SQLite E2E harness state: %w", err)
		}
	...
	if !committed {
		...
		record, err := h.state.Append(ctx, session.Control.ResumeRef, revision, "application/vnd.compforge.e2e.input+json", data)
		if err != nil {
			return app.TurnResult{ResumeRevision: revision}, fmt.Errorf("append SQLite E2E harness state: %w", err)
		}
		revision = record.Revision
	}
	if h.block {
		...
		<-ctx.Done()
		return app.TurnResult{ResumeRevision: revision}, ctx.Err()
	}

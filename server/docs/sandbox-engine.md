...
Sandbox Engine 是 agentd 与隔离执行环境之间的稳定边界。agentd 不实现容器、虚拟机、文件系统或
远端执行服务，只通过 Engine 契约为 Harness 提供文件操作和命令执行能力。
...
生命周期与执行面的划分，但不把 OpenSandbox 作为对外协议或默认实现。Go `Engine` 是 agentd 自己的
能力契约，不复制 OpenAPI，也不把具体服务的 HTTP 路由暴露给 Controller 或 Harness。

这条边界把长期身份与短期计算资源分开：

- **Environment** 描述 Session 需要的运行环境，不是一台已经启动的沙箱；
- **Session** 是用户看到的长期身份，可以跨 agentd、Harness runtime 和 Sandbox instance 存在；
- **Sandbox Engine** 负责把逻辑 Sandbox 身份解析为可用的隔离 Workspace；
- **Sandbox instance** 是 Engine 管理的可释放资源，可以是 Hostel Bed、容器、虚拟机或远端实例。

agentd 将 Sandbox provider 返回的 ID 视为不透明资源引用。当前 Hostel Adapter 直接以 Session ID
作为 Bed ID；由服务端分配 Sandbox ID 的 Engine 应将其保存为 Control State 中的资源绑定，或通过
稳定 metadata 重新解析。Provider ID 不能取代 Session ID 成为产品身份。

## 所有权边界

| 组件 | 拥有什么 | 不拥有什么 |
| --- | --- | --- |
| agentd Session Controller | Session 生命周期、执行时机、恢复与资源释放决策 | 沙箱内部实现、命令语义 |
...
记录 `tool.requested`、`tool.completed` 或失败事实；结果不明确时，由 agentd 结合 Harness State 和
Ledger 决定对账、终止或恢复，不能让 Engine 静默重放命令。
...
| --- | --- | --- |
| Lifecycle | 创建、查询、暂停、恢复、续期和删除 Sandbox；从 image 或 snapshot 启动 | Session Controller 管理资源绑定和保留策略 |
...
`sandbox.Engine` 仍作为 Controller/Harness 的稳定依赖边界，对外实现则可以直接接入兼容的本地或远端
Sandbox 服务。这样既降低 Engine 接入成本，也让 OpenSandbox 生态中的部署、工具和供应商能够直接
成为 agentd 的可选运行环境。
...
Controller 与 Harness。所有实现使用同一组 Engine 契约测试验证 Workspace、命令执行、超时取消、
...
`internal/sandbox.Engine` 暴露 agentd 当前需要的最小能力：
...
只使用 `/workspace` 内的路径，不依赖 agentd 所在主机的本地路径。
...
durable user Event
        │
        ▼
agentd wakes Session and restores Harness State
        │
        ▼
...
Engine 可以在 `Ensure` 背后启动本地资源，也可以调用远端服务。agentd 只观察契约结果，不要求 Engine
与自己同进程、同主机或共享文件系统。
...
1. agentd 重启后通过持久化的资源绑定调用 `Ensure`，重新解析 endpoint，不缓存临时实例地址；
...
4. 未决 Tool Attempt 是否可重放由 Ledger 和 Controller 判断，Workspace 中出现文件不能单独证明命令成功；
5. `Ensure` 只负责资源就绪，不授予 Session 执行所有权，也不能替代多实例所需的 lease 和 fencing。

当前 agentd 是单实例 Controller。未来多个 agentd 共享同一 Engine 时，执行所有权仍由 Control State
维护；在所有权 token 能进入 Engine 调用链之前，不能依赖 Sandbox 服务自行避免并发执行。
...
- **网络边界**：通过 Environment 映射 OpenSandbox network policy，不继承 agentd 主机权限；
...
这些要求由具体 Engine 和部署环境落实。agentd 负责配置、调用和验证能力，不用一个宽泛的 Engine
接口掩盖实现无法提供的隔离或持久性保证。

## Hostel Adapter

Hostel 的设计参考了 OpenSandbox，`internal/hostel` 是 agentd 当前唯一的 `sandbox.Engine` 实现：
...
OpenSandbox 的 Sandbox、Execution 和 Workspace 设计，但只需实现 agentd 的 `sandbox.Engine` 契约并
复用 Sandbox/Harness 契约测试，不让 provider 专属类型和 HTTP header 进入 Controller 与 Harness。

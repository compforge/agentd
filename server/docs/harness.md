...
Harness 是 agentd 接入 Agent Loop 的执行边界。它负责模型循环、原生上下文和工具调用语义，
但不拥有 Session 生命周期、调度策略或沙箱实现。

当前首个实现是进程内的 AgentGo Adapter。agentd 通过稳定的 Harness 契约调用它，避免让
Controller、HTTP API 和持久化层依赖 AgentGo 类型；后续接入其它本地或远端 Harness 时，
...
| 组件 | 拥有 | 不拥有 |
| --- | --- | --- |
| Session Controller | 输入持久化、唤醒、运行状态、重试与终止决策 | 模型循环、原生上下文格式 |
...
| `PrepareSession` | 为新 Session 准备原生状态，并返回对 Controller 不透明的 `ResumeRef` |
...
Managed Event，由 agentd 持久化。接口不暴露原生 message、model、tool 或远端 session 类型。

当前一个 agentd 进程只组装一个 Harness 实现，新建 Session 时将其名称和版本固定到 Control
State。未来支持多个 Harness 时，应根据 Session 中已固定的身份选择实现，不能在恢复时静默切换。
...
durable user Event
        │
        ▼
Session Controller marks running
        │
        ▼
...
commit native state + project Managed Event
        │
        ▼
Controller records revision and returns Session to idle
```
...
Harness State 由对应 Adapter 解释。agentd 只保存 `ResumeRef` 和 revision，不要求不同 Harness
转换成统一消息或 checkpoint schema。当前 AgentGo Adapter 使用版本化 opaque record 追加已提交的
原生 message，并以 revision 做乐观并发检查。
...
4. 无法证明结果的 Tool Attempt 不自动重放，而是返回 `ErrUnsafeRecovery` 交给 Controller 终止并对账。
...
这两类集成属于 Adapter，不应进入 Session Controller。Controller 只消费 Harness 的运行结果、
持久化输出和明确的恢复错误。
...
1. Session 的长期身份和执行时机属于 agentd，不能绑定某个 Harness runtime。
2. Harness 原生类型和状态格式不能泄漏到 API、Controller、Ledger 协议或 Sandbox Engine。
3. Session 固定的 Harness 名称与版本必须参与恢复兼容性判断。
4. Harness 输出先成为可幂等识别的 Managed Event，再由 agentd 对外提供。

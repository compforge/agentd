...
func (c *Client) Run(ctx context.Context, bedID string, command sandbox.Command) (sandbox.CommandResult, error) {
	...
	response, err := c.httpClient.Do(request)
	if err != nil {
		return sandbox.CommandResult{}, fmt.Errorf("run hostel command: %w", err)
	}
	defer response.Body.Close()
	if response.StatusCode != http.StatusOK {
		return sandbox.CommandResult{}, responseError("run hostel command", response)
	}
	...
	for scanner.Scan() {
		...
		if err := json.Unmarshal([]byte(line), &event); err != nil {
			return sandbox.CommandResult{}, fmt.Errorf("decode hostel command event: %w", err)
		}
		switch event.Type {
		case "stdout", "stderr":
	...
	if err := scanner.Err(); err != nil {
		return sandbox.CommandResult{}, fmt.Errorf("read hostel command stream: %w", err)
	}
	return result, nil
}

func (c *Client) Stat(ctx context.Context, bedID, filePath string) (sandbox.FileInfo, error) {
	if err := c.EnsureBed(ctx, bedID); err != nil {
	...
	response, err := c.httpClient.Do(request)
	if err != nil {
		return sandbox.FileInfo{}, fmt.Errorf("stat hostel file %q: %w", filePath, err)
	}
	defer response.Body.Close()
	if response.StatusCode == http.StatusNotFound {
		return sandbox.FileInfo{}, fs.ErrNotExist
	}
	if response.StatusCode != http.StatusOK {
		return sandbox.FileInfo{}, responseError("stat hostel file", response)
	}
	var values map[string]fileInfo
	if err := json.NewDecoder(response.Body).Decode(&values); err != nil {
		return sandbox.FileInfo{}, fmt.Errorf("decode hostel file info: %w", err)
	}
	value, ok := values[filePath]
	if !ok {
		return sandbox.FileInfo{}, fs.ErrNotExist
	}
	return value.sandbox(), nil
}
...
func (c *Client) ReadDir(ctx context.Context, bedID, directory string) ([]sandbox.DirEntry, error) {
	if err := c.EnsureBed(ctx, bedID); err != nil {
		return nil, err
	}
	...
	entries := make([]sandbox.DirEntry, 0, len(values))
	for _, value := range values {
		if path.Clean(value.Path) == path.Clean(directory) {
			continue
		}
		entries = append(entries, sandbox.DirEntry{Name: path.Base(value.Path), IsDir: value.Type == "directory"})
	}
	return entries, nil
}
...
func (f fileInfo) sandbox() sandbox.FileInfo {
